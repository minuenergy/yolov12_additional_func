...
def output_to_rotated_target(output, max_det=300):
    ...
    Assumes extended format with top-3 class info, but slices only needed parts.
    """
    targets = []
    for i, o in enumerate(output):
        # Only keep required columns: [x, y, w, h, angle, conf, class]
        if o.shape[1] >= 7:
            box = o[:max_det, :4].cpu()               # x, y, w, h
            angle = o[:max_det, 4:5].cpu()            # angle
            conf = o[:max_det, 5:6].cpu()             # best confidence
            cls = o[:max_det, 6:7].cpu()              # best class
        else:
            continue  # skip if shape doesn't match

        j = torch.full((conf.shape[0], 1), i)         # batch idx
        targets.append(torch.cat((j, cls, box, angle, conf), 1))
    if not targets:
        return [], [], [], []
